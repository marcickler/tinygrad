...
class HFEmbedder:
    def __init__(self, version: str, max_length: int, **hf_kwargs):
        ...
        if self.is_clip:
            self.tokenizer = FrozenOpenClipEmbedder(version, max_length=max_length)
            self.hf_module = FrozenOpenClipEmbedder(version, **hf_kwargs)
        else:
            pass
...
class RMSNorm:
    def __init__(self, dim: int):
        super().__init__()
        self.scale = Tensor.ones(dim, requires_grad=True)

    def __call__(self, x: Tensor):
        x_dtype = x.dtype
        x = x.float()
        rrms = (x**2).mean(dim=-1, keepdim=True).add(1e-6).rsqrt()
...
class LastLayer:
    def __init__(self, hidden_size: int, patch_size: int, out_channels: int):
        ...
        self.adaLN_modulation = nn.Sequential(Tensor.silu, nn.Linear(hidden_size, 2 * hidden_size, bias=True))
...
if __name__ == "__main__":
    ...
    for k, v in weights.items():
        print(k, v.shape)
    clip = FrozenOpenClipEmbedder(1024, 16, 24, False, True)
    our_state_dict = get_state_dict(clip)
    for k, v in our_state_dict.items():
        print(k, v.shape)
    load_state_dict(clip, weights, strict=True)
    ...
    # print(len(tiny_state_dict_keys))
    # print(len(torch_state_dict_keys))
    # counter = 0
    # for key in new_state_dict:
    #     if key not in tiny_set:
    #         # print(key)
    #         counter += 1
    #     else:
    #         tiny_set.remove(key)
    # tiny_keys = sorted(list(tiny_set))
    # for key in tiny_keys:
    #     print(key)
    # print(counter)
    # #print(sorted(torch_state_dict_keys))
    # #print(sorted(tiny_state_dict_keys))